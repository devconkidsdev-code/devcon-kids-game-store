...
app.post("/api/gemini/flood-risk-assessment", async (req, res) => {
  try {
    ...
    if (!ai) {
      return res.json({
        summary: language === "tl" ? "Matinding pagbabantay sa Calumpit dahil sa pagsasama ng Habagat at pagpapakawala ng tubig mula sa Bustos Dam. Pinapayuhan ang agarang paglikas sa mabababang barangay tulad ng Meysulao, Frances, at Gatbuca." : "Critical flood monitoring active across Calumpit basin. Confluence of Pampanga and Angat rivers combined with Bustos Dam spilling requires urgent preemptive evacuations in riverside and low-lying barangays.",
...
app.post("/api/gemini/resident-advisor", async (req, res) => {
  try {
    ...
    if (!ai) {
      return res.json({
        reply: language === "tl" ? `Para sa inyong kaligtasan sa Barangay ${residentBarangay || "Calumpit"}: Kung ang tubig ay patuloy na tumataas, agad makipag-ugnayan sa Calumpit MDRRMO sa (044) 913-7288 o 0917-800-MDRR. Ihanda ang inyong Go-Bag, patayin ang kuryente, at lumikas sa pinakamalapit na evacuation center habang passable pa ang daan.` : `For residents in Barangay ${residentBarangay || "Calumpit"}: If water levels are rising rapidly, contact Calumpit MDRRMO at (044) 913-7288. Turn off main circuit breakers, secure emergency kits, and proceed to the designated high-ground shelter immediately.`
      });
    }
